...
func main() {
	...
	streamTool, err := searxng.BuildSearchStreamTool(&searxng.ClientConfig{
		BaseUrl:    searxngURL,
		Timeout:    30 * time.Second,
		MaxRetries: 3,
	})
	...
	// 准备流式搜索参数
	streamLanguage := "zh-CN"     // 使用中文搜索
	streamEngines := "baidu,bing" // 使用不同的搜索引擎
	streamReq := searxng.SearchRequest{
		Query:    "CloudWeGo Eino",
		PageNo:   1,
	...
	streamArgs, err := json.Marshal(streamReq)
	if err != nil {
		log.Fatal(err)
	}

	// 执行流式搜索
	streamResp, err := streamTool.StreamableRun(ctx, string(streamArgs))
	if err != nil {
		log.Fatal(err)
	}
