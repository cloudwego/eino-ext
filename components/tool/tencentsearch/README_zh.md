...
- `Mode`: 返回结果类型，`0`-自然检索，`1`-多模态VR，`2`-混合 (默认: `0`)。未设置时使用默认值 `0`；显式传入其他值会在初始化时报错。
- `Cnt`: 每次请求默认返回的搜索结果条数。未设置时默认使用 `10`；显式传入时仅允许 `10/20/30/40/50`，其他值会在初始化时报错。
...
- `Mode`: 可选返回结果类型，`0` 自然检索，`1` 多模态VR，`2` 混合。未设置时沿用配置值或默认值 `0`；显式传入其他值会直接报错。
- `Site`: 可选站内搜索域名。
- `FromTime`: 可选起始时间，秒级 Unix 时间戳。
- `ToTime`: 可选结束时间，秒级 Unix 时间戳。
- `Industry`: 可选行业过滤参数，可选值为 `gov/news/acad/finance`，分别表示党政机关、权威媒体、学术（英文）、金融；
- `Cnt`: 可选返回结果数量。未设置时沿用配置值或默认值 `10`；显式传入时仅允许 `10/20/30/40/50`，其他值会直接报错。
...
- `Mode` 参数仅支持 `0/1/2`
- `Industry` 参数仅尊享版支持
- 工具会在调用 API 前直接拒绝非法的 `Cnt` 或 `Mode` 显式取值
