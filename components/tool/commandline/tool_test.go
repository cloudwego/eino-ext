...
// MockFileOperator mocks the FileOperator interface
...
func TestNewStrReplaceEditor(t *testing.T) {
	...
	editor, err := NewStrReplaceEditor(ctx, &Config{Operator: mockOperator})
...
func TestStrReplaceEditor_ValidatePath(t *testing.T) {
	...
	// Create editor
	editor, _ := NewStrReplaceEditor(ctx, &Config{Operator: mockOperator})
