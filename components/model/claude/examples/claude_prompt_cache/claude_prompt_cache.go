...
func systemCache() {
	...
	// SetMessageBreakpoint with WithCacheTTL sets a manual cache breakpoint with a 1-hour TTL.
	...
	}, claude.WithCacheTTL(claude.CacheTTL1h))
...
func toolInfoCache() {
	...
	// SetToolInfoBreakpoint with WithCacheTTL sets a manual cache breakpoint on the tool with a 5-minute TTL.
	breakpoint := claude.SetToolInfoBreakpoint(&schema.ToolInfo{
		...
		claude.WithCacheTTL(claude.CacheTTL5m))
...
func sessionCache() {
	...
	// WithEnableAutoCache with WithAutoCacheTTL enables automatic caching with a 1-hour TTL.
	opts := []model.Option{
		claude.WithEnableAutoCache(true),
		claude.WithAutoCacheTTL(claude.CacheTTL1h),
	}
