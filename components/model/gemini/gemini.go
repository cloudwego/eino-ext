...
func (c *chatModel) Generate(ctx context.Context, input []*schema.Message, opts ...model.Option) (message *schema.Message, err error) {
	session, conf := c.initGenerativeModelSession(opts...)
...
func (c *chatModel) BindTools(tools []*schema.ToolInfo) error {
	...
	for i, tool := range tools {
		...
		funcDecl.Parameters = c.convOpenSchema(openSchema)
...
func (c *chatModel) initGenerativeModelSession(opts ...model.Option) (*genai.ChatSession, *model.Config) {
	...
	if geminiOptions.ResponseSchema != nil {
		...
		m.ResponseSchema = c.convOpenSchema(geminiOptions.ResponseSchema)
	}
	return m.StartChat(), conf
}

func (c *chatModel) convOpenSchema(schema *openapi3.Schema) *genai.Schema {
	if schema == nil {
		return nil
	}
	...
	case openapi3.TypeObject:
		...
		if schema.Properties != nil {
			...
			for name, prop := range schema.Properties {
				...
				properties[name] = c.convOpenSchema(prop.Value)
			}
	...
	case openapi3.TypeArray:
		...
		if schema.Items != nil && schema.Items.Value != nil {
			result.Items = c.convOpenSchema(schema.Items.Value)
		}
