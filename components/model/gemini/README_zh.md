...
Gemini 返回原生函数调用 ID 时，本实现会将其保留在 Eino 消息中，并在对应的函数结果中原样传回。因此，即使多个并行调用使用同一个函数且结果乱序返回，也能通过 ID 正确匹配。

对于不返回 ID 的旧模型或旧网关，本实现仍会生成唯一的 UUID（v4），并在函数调用及其结果回传 Gemini 时复用该 ID。

**示例：**
```go
// 如果 Gemini 为不同城市返回多次 "get_weather" 调用：
// 工具调用 1：ID = "provider-call-1", Args = {"city": "Paris"}
// 工具调用 2：ID = "provider-call-2", Args = {"city": "London"}
// 结果可以按任意顺序返回，因为每个结果都携带对应的 ID。
```
