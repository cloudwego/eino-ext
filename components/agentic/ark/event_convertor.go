...
func (r *streamReceiver) getReasoningSummaryIndex(outputIdx, summaryIdx int64) int {
	maxSummaryIndex := -1
	...
	if idx, ok := r.ReasoningSummaryIndexMapper[idxKey]; ok {
		return idx
	}
...
func (r *streamReceiver) itemDoneEventFunctionMCPCallToContentBlocks(outputIdx int64, item *responses.OutputItem_FunctionMcpCall) (blocks []*schema.ContentBlock, err error) {
	...
	for _, block := range blocks {
		...
		default:
			return nil, fmt.Errorf("expected mcp tool call or result block, but got '%s'", block.Type)
		}
	}
