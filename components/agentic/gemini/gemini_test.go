...
func TestNewAgenticModel(t *testing.T) {
	...
	assert.True(t, model.enableCodeExecution)
	...
	assert.Equal(t, schema.ToolChoiceAllowed, *model.toolChoice)
}

func TestGemini_GenInputAndConf(t *testing.T) {
	originalTemp := float32(0.5)
	g := &gemini{
		cli:         &genai.Client{},
		model:       "gemini-pro",
	...
	modelName, _, genaiConf, conf, err := g.genInputAndConf(
		input,
		agentic.WithTemperature(newTemp),
		agentic.WithModel(newModel),
		agentic.WithToolChoice(schema.ToolChoiceForced),
		agentic.WithTools([]*schema.ToolInfo{{}}),
	)
...
func TestGemini_Stream_Success(t *testing.T) {
	...
	assert.NotNil(t, chunks[0].ContentBlocks[0].StreamMeta)
	assert.Equal(t, 0, chunks[0].ContentBlocks[0].StreamMeta.Index)
...
func TestConvCallbackOutput(t *testing.T) {
	o := convCallbackOutput(&schema.AgenticMessage{}, &agentic.Config{})
