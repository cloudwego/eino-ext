...
var _ agentic.Model = (*gemini)(nil)
...
func (g *gemini) CreatePrefixCache(ctx context.Context, prefixMsgs []*schema.AgenticMessage, opts ...agentic.Option) (
	*genai.CachedContent, error) {
	...
	contents, err := g.convAgenticMessages(inputMsgs)
	if err != nil {
		return nil, err
	}
...
func (g *gemini) Generate(ctx context.Context, input []*schema.AgenticMessage, opts ...agentic.Option) (*schema.AgenticMessage, error) {
	...
	co := agentic.GetCommonOptions(&agentic.Options{
		Tools:      g.origTools,
	...
	contents, err := g.convAgenticMessages(nInput)
	if err != nil {
		return nil, err
	}
...
func (g *gemini) Stream(ctx context.Context, input []*schema.AgenticMessage, opts ...agentic.Option) (*schema.StreamReader[*schema.AgenticMessage], error) {
	ctx = callbacks.EnsureRunInfo(ctx, g.GetType(), components.ComponentOfChatModel)
	...
	go func() {
		...
		for resp, err_ := range resultIter {
			...
			curIndex, lastType = populateStreamMeta(message.ContentBlocks, curIndex, lastType)
	...
	return schema.StreamReaderWithConvert(srList[1], func(t *agentic.CallbackOutput) (*schema.AgenticMessage, error) {
		return t.Message, nil
	}), nil
}

func (g *gemini) WithTools(tools []*schema.ToolInfo) (agentic.Model, error) {
	if len(tools) == 0 {
