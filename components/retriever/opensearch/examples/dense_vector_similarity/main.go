...
import (
	...
	"github.com/cloudwego/eino-ext/components/retriever/opensearch/search_mode"
)
...
func main() {
	...
	ret, err := opensearch.NewRetriever(ctx, &opensearch.RetrieverConfig{
		Client: client,
		Index:  indexName,
		// Demonstrate DenseVectorSimilarity search mode
		SearchMode: search_mode.DenseVectorSimilarity(search_mode.DenseVectorSimilarityTypeCosineSimilarity, "vector"),
		Embedding:  mockEmb,
	})
	if err != nil {
	...
	fmt.Printf("\n--- Retrieving for query: '%s' using DenseVectorSimilarity ---\n", query)
