...
func NewRange(metricType MetricType, radius float64) *Range {
	return &Range{
		MetricType: metricType,
		Radius:     radius,
	}
...
func (r *Range) Retrieve(ctx context.Context, client *milvusclient.Client, conf *RetrieverConfig, query string, opts ...retriever.Option) ([]*schema.Document, error) {
...
func (r *Range) BuildSearchOption(ctx context.Context, conf *RetrieverConfig, queryVector []float32, opts ...retriever.Option) (milvusclient.SearchOption, error) {
	...
	if r.RangeFilter != nil {
		searchOpt.WithSearchParam("range_filter", fmt.Sprintf("%v", *r.RangeFilter))
	}
