...
func (h *Hybrid) BuildSearchOption(ctx context.Context, conf *RetrieverConfig, queryVector []float32, opts ...retriever.Option) (milvusclient.SearchOption, error) {
	return nil, fmt.Errorf("Hybrid search mode requires BuildHybridSearchOption")
}

// Retrieve performs the hybrid search operation.
func (h *Hybrid) Retrieve(ctx context.Context, client *milvusclient.Client, conf *RetrieverConfig, query string, opts ...retriever.Option) ([]*schema.Document, error) {
...
func (h *Hybrid) BuildHybridSearchOption(ctx context.Context, conf *RetrieverConfig, queryVector []float32, query string, opts ...retriever.Option) (milvusclient.HybridSearchOption, error) {
	...
	for _, req := range h.SubRequests {
		...
		if field == "" {
			if req.VectorType == SparseVector {
		...
		// Apply search params: Config first, then struct overrides
		for k, v := range ExtractSearchParams(conf, field) {
