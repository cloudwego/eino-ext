...
// This is a narrower interface than SearchClient because batch indexing uses
// pipeline commands (HSET, JSON.SET) rather than individual FT.SEARCH calls.
...
func (i *Indexer) batchHSet(ctx context.Context, docs []*schema.Document, options *indexer.Options) error {
	...
	embAndStore := func() error {
		...
		for _, t := range tuples {
			...
			batch.HSet(i.config.KeyPrefix+t.key, t.fields)
		}

		if _, err := i.config.Client.Exec(ctx, *batch, true); err != nil {
			return err
		}
...
func (i *Indexer) batchJSONSet(ctx context.Context, docs []*schema.Document, options *indexer.Options) error {
	...
	flushBatch := func() error {
		...
		for idx, p := range pending {
			...
			batch.CustomCommand([]string{"JSON.SET", i.config.KeyPrefix + p.doc.ID, "$", string(jsonBytes)})
		}
