...
func (d *debugServiceImpl) DebugRun(ctx context.Context, rm *model.DebugRunMeta, userInput string, inputFormat types.InputFormat) (debugID string,
	...
	if strings.Contains(userInput, "test_json") {
		inputFormat = types.InputFormatOfJson
	} else {
		inputFormat = types.InputFormatOfCode
		userInput = `
		var input = NodeInfo{
			Message: "input",
		}
	...
	if inputFormat == types.InputFormatOfCode {
		...
		jsonInput, err := model.ConvertCodeToJsonData(userInput, nodeInfo.ComponentSchema.InputType)
		if err != nil {
			return "", nil, nil, err
		}
		println(jsonInput)
