...
func TestLsInfo(t *testing.T) {
	...
	s, err := NewLocalBackend(ctx, &Config{})
	assert.NoError(t, err)
...
func TestExecute(t *testing.T) {
	ctx := context.Background()
	lsb, err := NewLocalBackend(ctx, &Config{})
	assert.NoError(t, err)
	s, _ := lsb.(filesystem.ShellBackend)

	t.Run("Execute echo", func(t *testing.T) {
		req := &filesystem.ExecuteRequest{Command: "echo hello world"}
	...
	t.Run("Execute failure", func(t *testing.T) {
		req := &filesystem.ExecuteRequest{Command: "exit 1"}
		_, err := s.Execute(ctx, req)
		assert.Error(t, err)
		assert.Contains(t, err.Error(), "command exited with non-zero code")
	})

	t.Run("Execute stderr", func(t *testing.T) {
		req := &filesystem.ExecuteRequest{Command: "echo error >&2; exit 1"}
		_, err := s.Execute(ctx, req)
		assert.Error(t, err)
...
func TestExecuteStreaming(t *testing.T) {
	...
	t.Run("ExecuteStreaming with echo", func(t *testing.T) {
		...
		sr, err := s.(*localBackend).ExecuteStreaming(ctx, req)
		assert.NoError(t, err)
	...
	t.Run("ExecuteStreaming with context cancellation", func(t *testing.T) {
		...
		sr, err := s.(*localBackend).ExecuteStreaming(cancelCtx, req)
		assert.NoError(t, err)
	...
	t.Run("ExecuteStreaming with empty command", func(t *testing.T) {
		...
		_, err := s.(*localBackend).ExecuteStreaming(ctx, req)
		assert.Error(t, err)
