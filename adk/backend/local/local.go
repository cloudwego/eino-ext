...
func (s *Local) ExecuteStreaming(ctx context.Context, input *filesystem.ExecuteRequest) (result *schema.StreamReader[*filesystem.ExecuteResponse], err error) {
	...
	if input.RunInBackendGround {
		s.runCmdInBackground(cmd, stdout, stderr, w)
		return sr, nil
	}
...
func (s *Local) runCmdInBackground(cmd *exec.Cmd, stdout, stderr io.ReadCloser, w *schema.StreamWriter[*filesystem.ExecuteResponse]) {
	go func() {
		defer func() {
			_ = stdout.Close()
			_ = stderr.Close()
