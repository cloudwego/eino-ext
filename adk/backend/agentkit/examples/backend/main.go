...
func main() {
	...
	backend, err := agentkit.NewSandboxBackend(config)
	if err != nil {
		log.Fatalf("Failed to create ArkSandboxBackend: %v", err)
	}
	fmt.Println("✓ ArkSandboxBackend initialized successfully")
	fmt.Println()
