...
func TestNewArkSandbox(t *testing.T) {
	t.Run("Success: ValidConfig", func(t *testing.T) {
		...
		ss, err := NewSandboxBackend(config)
		s := ss.(*sandboxBackend)
		...
		assert.Equal(t, 60, s.invokeTimeout)
	})
	...
	t.Run("Success: Defaults", func(t *testing.T) {
		...
		assert.Equal(t, 30, s.invokeTimeout)
	})
	...
	t.Run("Failure: MissingRequiredFields", func(t *testing.T) {
		...
		testCases := []struct {
			...
			{"MissingUserSessionID", &Config{AccessKeyID: baseConfig.AccessKeyID, SecretAccessKey: baseConfig.SecretAccessKey, ToolID: baseConfig.ToolID}, "UserSessionID is required"},
		}

		for _, tc := range testCases {
			t.Run(tc.name, func(t *testing.T) {
				_, err := NewSandboxBackend(tc.config)
				require.Error(t, err)
	...
	t.Run("Failure: InvalidRegion", func(t *testing.T) {
		...
		_, err := NewSandboxBackend(config)
		require.Error(t, err)
...
func setupTest(t *testing.T) (*sandboxBackend, *httptest.Server) {
