...
func TestCoordinatorGetSetAndDeleteLifecycle(t *testing.T) {
	ctx := context.Background()
	mr := miniredis.RunT(t)
	coord := newTestCoordinator(t, mr)

	got, ok, err := coord.Get(ctx, "key-a")
	require.NoError(t, err)
	require.False(t, ok)
	require.Nil(t, got)

	require.NoError(t, coord.Set(ctx, "key-a", []byte("value-a"), 0))
	...
	got, ok, err = coord.GetAndDelete(ctx, "key-a")
	require.NoError(t, err)
	require.True(t, ok)
	require.Equal(t, []byte("value-a"), got)
...
func TestCoordinatorSetRespectsTTL(t *testing.T) {
	ctx := context.Background()
	mr := miniredis.RunT(t)
	coord := newTestCoordinator(t, mr)

	require.NoError(t, coord.Set(ctx, "key-a", []byte("value-a"), time.Second))
